  ...
  <div style="outline: none;">
    ...
    <div class="image-list-container display-flex flex-wrap" v-if="images.length && multiple">
      <div class="image-list-item position-relative cursor-pointer" :class="image.highlight && 'image-highlight'" v-for="(image, index) in images" :key="index" @click="changeHighlight(index)">
        <div class="centered">
          <img class="show-img img-responsive" :src="image.alt">
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    props: {
      dragText: {
        type: String,
        default: 'glisser des photos (beaucoup)'
      },
      browseText: {
        type: String,
    ...
    computed: {
      imagePreview () {
        ...
        if (index > -1) {
          ...
          return this.images.length ? this.images[0].alt : ''
        }
      },
      imageDefault () {
        ...
        reader.onload = (e) => {
          ...
          if (dataURI) {
            if (!this.images.length) {
              this.images.push({nom: file.name, alt: dataURI, highlight: 1, default: 1})
              this.currentIndexImage = 0
            } else {
              this.images.push({nom: file.name, alt: dataURI, highlight: 0, default: 0})
            }
        ...
        reader.onload = (e) => {
          ...
          if (dataURI) {
            if (this.images.length && this.images[this.currentIndexImage]) {
              this.images[this.currentIndexImage].alt = dataURI
              this.images[this.currentIndexImage].nom = file.name
            }
          }
        }
        reader.readAsDataURL(file)
