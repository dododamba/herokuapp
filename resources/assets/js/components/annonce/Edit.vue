		...
		<template slot="main">
			...
			<div class="form-group">
				...
				<textarea @keydown="delete errors.contenue" class="form-control" id="contenue"
						  name="contenue"
						  placeholder="Contenue de l'annonce" rows="9" v-model="editRec.contenue"></textarea>
			...
			<div class="row">
				<div class="form-group col-6">
					...
					<input type="date" id="dateDebut" name="dateDebut" placeholder="début d'annonce" v-model="editRec.dateDebut" class="form-control"><small class="form-text text-muted">début d'annonce</small>

				</div>
				<div class="form-group col-6">
					<label for="dateFin" class=" form-control-label"><strong>Date Fin</strong> </label>
					<input type="date" id="dateFin" name="dateFin" placeholder="date de fin de l'annonce"  v-model="editRec.dateFin"  class="form-control"><small class="form-text text-muted">fin d'annonce</small>
...
<script>
	...
	export default {
		...
		methods: {
			updateRecord() {
				axios.put(`/api/annonce/${this.editRec.slug}`, this.editRec)
					.then(response => {
